...
/** CTA de leitura: a leitura se agenda, não se compra. */
export function linkAgendar(nome: string, whatsapp?: string): string {
  return linkWhatsApp(`Oi! Vi no site e quero saber mais sobre: ${nome}.`, whatsapp);
}

export function linkGeral(whatsapp?: string): string {
  return linkWhatsApp("Oi! Vim pelo site da NYLIFE Nutrition.", whatsapp);
}
